...
export default {
  ...
  methods: {
    ...
    _renderWrapper(children) {
      return (
        <div
          ...
          class={[
            "yumy-wrapper",
            `yumy-wrapper--theme-${this.theme}`,
            { "yumy-wrapper--simple": this.simple },
            this.drawerVisible && "yumy-wrapper--drawer-show",
          ]}
        >
          {children}
        </div>
    ...
    _renderMenu() {
      ...
      return (
        <div class="yumy-menu" v-show={!this.hideMenu}>
          {
            <yumy-avatar
              ...
              class="yumy-menu__avatar"
              src={this.user.avatar}
            />
          }
          {menuItem.top}
          {this.$slots.menu}
          <div class="yumy-menu__bottom">
      ...
      this.menus.forEach(item => {
        ...
        const node = (
          <div
            class={[
              "yumy-menu__item",
              { "yumy-menu__item--active": this.activeSidebar == name },
            ]}
            on-click={() => {
              funCall(click, () => {
            ...
            title={title}
          >
            <yumy-badge count={unread}>{render(item)}</yumy-badge>
          </div>
        );
    ...
    _renderContact(props, onClick, slot) {
      ...
      return (
        <yumy-contact
          class={{
            "yumy-contact--active": this.currentContactId == props.contact.id,
          }}
          v-yumy-contextmenu_contact={this.contactContextmenu}
          props={props}
          on-click={click}
          ...
          this.contacts.map(contact => {
            ...
            const node = [
              contact.index !== prevIndex && (
                <p class="yumy-sidebar__label">{contact.index}</p>
              ),
              this._renderContact(
                {
                  contact: contact,
    ...
    _renderSidebar(children, name, fixedtop) {
      return (
        ...
        >
          <div class="yumy-sidebar__fixed-top">{fixedtop}</div>
          <div class="yumy-sidebar__scroll">{children}</div>
        </div>
      );
    },
    _renderDrawer() {
      return this._menuIsMessages() && this.currentContactId ? (
        <div class="yumy-drawer" ref="drawer">
    ...
    _renderContainer() {
      ...
      nodes.push(
        ...
        >
          <div class="yumy-container__title" style="border-bottom: 1px solid #bbb">
            {useScopedSlot(
              this.$scopedSlots["message-title"],
              <div class="yumy-container__displayname">
                {curact.displayName}
              </div>,
              curact,
            )}
          </div>
          <div class="yumy-vessel">
            <div class="yumy-vessel__left">
              <yumy-messages
                ref="messages"
      ...
      nodes.push(
        ...
        >
          {useScopedSlot(
            ...
            <div class="yumy-contact-info">
              <yumy-avatar src={curact.avatar} size={90} />
    ...
    /**
     * 初始化编辑框的 Emoji 表情列表，是 yumy-editor.initEmoji 的代理方法
      ...
      const defaultMenus = [
        {
          ...
          // eslint-disable-next-line no-unused-vars
          render: menu => {
            return <i class="yumy-icon-message" />;
          },
          isBottom: false,
        },
        {
          ...
          // eslint-disable-next-line no-unused-vars
          render: menu => {
            return <i class="yumy-icon-addressbook" />;
          },
          isBottom: false,
        },
      ];
...
+b(yumy-menu)
  ...
  +e(item)
    ...
    +m(active)
      color #43A047
    &:hover:not(.yumy-menu__item--active)
      color #eee
    word-break()
    > *
      font-size 24px
...
+b(yumy-contact-info)
  ...
  h4
    ...
    user-select none
.yumy-wrapper--theme-blue
  .yumy-message__content
    background #f3f3f3
    &::before
      border-right-color #f3f3f3
  .yumy-message--reverse .yumy-message__content
    background #e6eeff
    &::before
